...
доработками. Пакет: `skymot.winch`, версия **1.0.3**.

> APK для установки и полная инструкция по повторной сборке — в **Releases**
> этого репозитория (файл `Skymot-1.0.3.apk` + `upgrade.md`).
...
|---|---|---|
| **FAST** (красная) | `#D32F2F` | Нажата: `COMM_SET_RPM 20000` (20000 ERPM, повтор каждые 100 мс). Трос идёт легко — ток малый. Ток > 150 А — переключение на удержание `COMM_SET_CURRENT 150`. Разгон выше 20000 ERPM в режиме тока — возврат в скоростной режим (VESC сам снижает ток). Отпущена: `COMM_SET_CURRENT 0` |
| **SLOW** (зелёная) | `#388E3C` | Нажата: `COMM_SET_RPM 3000` (3000 ERPM, повтор каждые 100 мс). Ток > 150 А — аварийный стоп. Отпущена: стоп |
...
- **Защита FAST**: `fastCurrentMode` — при токе > 150 А приложение переходит
  ...
  выше 20000 ERPM (`msToErpm()` по конфигу: poles/gear/D) возвращается в RPM.
...
- `versionName` 1.0.3, `versionCode` 103
...
`VT_VERSION = 1.0.3`, коды Android-версий 103. Сборка целевая — **arm64-v8a**
...
- Кнопки FAST/SLOW — тестовые: только при тросе, уложенном на земле
  (20000 ERPM ≈ 38.8 м/с при poles=6, D=200 мм, gear=1.8).
