...
bool Skypuff::manualOverride() const
{
    return m_manualOverride;
}

void Skypuff::setManualOverride(bool v)
{
    if (m_manualOverride != v) {
        m_manualOverride = v;
        emit manualOverrideChanged(v);
    }
}
...
void Skypuff::timerEvent(QTimerEvent *event)
{
    if(event->timerId() == getConfTimeoutTimerId) {
        ...
        vesc->emitMessageDialog(tr("Command 'get_conf' timeout"),
                                tr("Skymot MCU did not answered within %2ms timeout.<br/><br/>"
                                   "Make sure connection is stable. VESC running Skymot enabled firmware. "
