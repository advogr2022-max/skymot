...
// so files are created via MediaStore (ContentResolver.insert + openOutputStream).
// Fallbacks: Android/data/skymot.winch/files, then internal files dir.
#include "logwriter.h"
...
static QFile g_fileGeneral;   // Download/Skymot (or Android/data fallback)
static QFile g_fileSmot;

// Try to open a log file. Preferred: Download/Skymot (user-visible).
// Fallback 1: Android/data/skymot.winch/files (file-manager visible via USB/MTP).
